...
Note that this app assumes you know the posterior distribution as well as the 
...
```{r beta-ci}
qbeta(c(0.05, 0.95), shape1 = 4, shape2 = 8)
```
...
```{r gamma-ci}
qgamma(c(0.025, 0.975), shape = 4, rate = 8).
```
...
```{r q9}
qbeta(c(0.05, 0.95), shape1 = 1+x2, shape2 = 1+n-x2)
```
