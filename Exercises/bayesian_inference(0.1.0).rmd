...
```{r for 0.1.0 version of statsr}
install.packages('/Users/shengnanyou/Desktop/statsr', repos = NULL, type="source")
library('statsr')
...
statsr::bayes_inference(y = weight, data = nc_fullterm, 
...
answer: Very Strong
```{r Q4-HT-weight}
a <- statsr::bayes_inference(y = weight, data = nc_fullterm, alternative = 'twosided', 
...
answer: there is a 95% chance that their birth weight will be 5.4 to 9.5 pounds.
```{r Q5-cred-int}
...
answer: Postive(questionable: Strong)
...
bayes_inference(y = weight, x = habit, data = nc_fullterm, hypothesis_prior = c(0.25, 0.75),
                statistic = "mean", 
...
bayes_inference(y = weight, x = habit, data = nc_fullterm, 
                statistic = "mean", 
                type = "ci", alternative = "twosided", mu_0 = 0, 
                prior = "JZS", rscale = 1, 
                method = "theoretical", show_plot = FALSE)
```
